// Copyright (C) 2019 Miromico AG. All rights reserved.
//
// Brief: HW description for MAX32625
...
#define IRQ_PIN_SET(gpio, on)	gpio_set_extirq(gpio, (on))
...
#define HW_CFG_PIN(p,i,cfg)	do { \
	...
	if ((((cfg) & GPIOCFG_MODE_MASK)  == (GPIOCFG_MODE_INP))   && \
		(((cfg) & GPIOCFG_OTYPE_MASK) == (GPIOCFG_OTYPE_PUPD)) && \
		(((cfg) & GPIOCFG_PUPD_MASK)  == (GPIOCFG_PUPD_PUP)))   {  \
		pad = GPIO_PAD_INPUT_PULLUP; \
	} \
	if ((((cfg) & GPIOCFG_MODE_MASK)  == (GPIOCFG_MODE_INP))   && \
		(((cfg) & GPIOCFG_OTYPE_MASK) == (GPIOCFG_OTYPE_PUPD)) && \
		(((cfg) & GPIOCFG_PUPD_MASK)  == (GPIOCFG_PUPD_PDN)))   {  \
		pad = GPIO_PAD_INPUT_PULLDOWN; \
	} \
	if ((((cfg) & GPIOCFG_MODE_MASK)  == (GPIOCFG_MODE_INP))   && \
		(((cfg) & GPIOCFG_OTYPE_MASK) == (GPIOCFG_OTYPE_OPEN))) {  \
 		pad = GPIO_PAD_INPUT; \
	} \
	if ((((cfg) & GPIOCFG_MODE_MASK)  == (GPIOCFG_MODE_OUT))   && \
		(((cfg) & GPIOCFG_OTYPE_MASK) == (GPIOCFG_OTYPE_OPEN)) && \
		(((cfg) & GPIOCFG_OSPEED_MASK) == (GPIOCFG_OSPEED_400kHz))) { \
		pad = GPIO_PAD_SLOW; \
	} \
	if ((((cfg) & GPIOCFG_MODE_MASK)  == (GPIOCFG_MODE_OUT))   && \
		(((cfg) & GPIOCFG_OTYPE_MASK) == (GPIOCFG_OTYPE_OPEN)) && \
		(((cfg) & GPIOCFG_OSPEED_MASK) == (GPIOCFG_OSPEED_2MHz))) { \
		pad = GPIO_PAD_NORMAL; \
	} \
	if ((((cfg) & GPIOCFG_MODE_MASK)  == (GPIOCFG_MODE_OUT))   && \
		(((cfg) & GPIOCFG_OTYPE_MASK) == (GPIOCFG_OTYPE_OPEN)) && \
		(((cfg) & GPIOCFG_OSPEED_MASK) == (GPIOCFG_OSPEED_10MHz))) { \
		pad = GPIO_PAD_NORMAL; \
	} \
	if ((((cfg) & GPIOCFG_MODE_MASK)  == (GPIOCFG_MODE_OUT))   && \
		(((cfg) & GPIOCFG_OTYPE_MASK) == (GPIOCFG_OTYPE_OPEN)) && \
		(((cfg) & GPIOCFG_OSPEED_MASK) == (GPIOCFG_OSPEED_40MHz))) { \
		pad = GPIO_PAD_FAST; \
	} \
	const gpio_cfg_t gpio = {(p), (1<<(i)), gpio_func, pad}; \
	uint32_t err = GPIO_Config(&gpio); \
	ASSERT(err==0); \
} while (0)

#define HW_SET_PIN(p,i,state)	do { \
	const gpio_cfg_t gpio = {(p), 1<<(i), GPIO_FUNC_GPIO, 0}; \
	if((state)==0){ \
		GPIO_OutClr(&gpio); \
	} \
	else { \
...
#define HW_GET_PIN(p,i)		(MXC_GPIO->in_val[(p)] & (1<<(i)))
...
#define DATA_EEPROM_BASE (0x0007C000)
#define DATA_EEPROM_END  (0x0007E000)
