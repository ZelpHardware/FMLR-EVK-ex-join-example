...
void uexti_handler (void) {
    unsigned int m0[5], m1[5];

    for (int i=0; i<5; i++){
    	m0[i] = m1[i] = MXC_GPIO->intfl[i];           // get pending interrupts
    }

    hook_uexti_irq(&m1);                              // call hooks

    for (int i=0; i<5; i++){
    	MXC_GPIO->intfl[i] = (m0[i] ^ m1[i]) & m0[i]; // clear handled interrupts
    }
}
