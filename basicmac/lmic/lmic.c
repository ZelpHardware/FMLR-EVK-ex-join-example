...
static void initJoinLoop (void) {
    ...
    if( REG_IS_FIX() ) {
        ...
        // Set the datarate depending on the join strategy
        setDrJoin(DRCHG_SET, os_getSlowestJoin() ? LMIC_slowestDr() : fastest125());
    }
    LMIC.txPowAdj = 0;
    LMIC.nbTrans = 0;
    LMIC.nbRetrans = 0;
...
nextblock:
	...
	delay = rndDelay(32);
        ...
        LMIC.opmode &= ~OP_NEXTCHNL;
        // Check for join strategy
        if (os_getSlowestJoin()) {
            // try 5 times on slowest dr before back off
            if (++LMIC.txCnt > 5) {
                failed = 1;
            }
        } else {
...
static bit_t decodeFrame (void) {
    ...
    while( oidx < olen ) {
        ...
        case MCMD_TIME_ANS: {
            u8_t secs = os_rlsbf4(&opts[oidx+1]);
...
static bit_t processDnData (void) {
      ...
      norx:
        ...
        if( (LMIC.opmode & OP_TXDATA) ) {
            ...
            if( LMIC.pendTxConf != 0 ) { // we requested an ACK
                ...
                if( LMIC.txCnt <= (LMIC.nbRetrans) ) {
                    // Schedule another retransmission with adjusted datarate
                    if (((LMIC.txCnt % 2) == 0) && (LMIC.adrEnabled)) {
                        setDrTxpow(DRCHG_NOACK, decDR(LMIC.datarate), 0);
                    }
                    txDelay(LMIC.rxtime, RETRY_PERIOD_secs);
                    LMIC.opmode = (LMIC.opmode & ~OP_TXRXPEND) | OP_NEXTCHNL;
                    os_setCallback(&LMIC.osjob, FUNC_ADDR(runEngineUpdate));
...
#ifdef CFG_NOBEACON
        // Dont start a scan and reset LMIC.ask_for_time if no answer is received, just ask again next time
#else
    ...
    if (!d) {
	  if( ((LMIC.txrxFlags & TXRX_DNW1) != 0) && ((LMIC.opmode & CLASS_C) != 0) ) {
		  return 0;
	  }
	  goto norx;
    }
...
void LMIC_setNbRetrans (uint8_t nbRetrans) {
    LMIC.nbRetrans = min(nbRetrans, maxNbRetrans);
}
