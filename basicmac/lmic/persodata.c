...
#define FLG_SLOWEST_JOIN        15

typedef struct {
    uint32_t    magic;          // 0x00 magic
    uint32_t    hwid;           // 0x04 hardware ID
    uint32_t    region;         // 0x08 region ID
    uint16_t    flags;          // 0x0c bit 15: 1 -> start join at min dr, 0 -> start join at max dr. Rest is RFU
    uint16_t    reserved;       // 0x0e (reserved, set to 0)
...
void pd_save(persodata_v1* pd) {
  persodata_v1 *ppd = (void*)PERSODATA_BASE;
  sha256(pd->hash, (uint8_t *)pd, sizeof(persodata_v1) - 32);
  eeprom_copy(ppd, pd, sizeof(persodata_v1));
}
...
bool hal_get_slowest_join (void) {
    return (bool) (pd.flags & (1 << FLG_SLOWEST_JOIN));
}

void hal_set_slowest_join (bool enable) {
    if (enable) {
        pd.flags |= (1 << FLG_SLOWEST_JOIN);
    } else {
        pd.flags &= ~(1 << FLG_SLOWEST_JOIN);
    }
    pd_save(&pd);
...
// provide device join strategy
bool os_getSlowestJoin (void) {
    return hal_get_slowest_join();
}

// change the join strategy
void os_setSlowestJoin (bool enable) {
    hal_set_slowest_join(enable);
}
#endif
